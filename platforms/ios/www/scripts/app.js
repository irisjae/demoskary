...
(function (self, args, my) {
		...
		self
			...
			.establish ('num-of-players', dependent (function () {
				return my ('match') .match_type_value;
			}, self .impressions ('match')))
			...
			.establish ('apply-numbers', dependent (function () {
				return my ('match') .applied_opponent_count;
			}, self .impressions ('match')))
...
(function (self, args, my) {
		...
		self
			...
			.establish ('field-type', dependent (function () {
				return field_type_to_chi (my (args .match__from) .pitch_type);
			}, self .impressions (args .match__from)))
...
(function (self, args, my) {
		...
		self
			.impressions (args .cycle__from)
				.thru (tap, function () {
					if (! self .affiliated ('::test-user')) {
						alert ('Cant hack')
						window .location .hash = home_path
					}
					else {
						inquire (self .affiliated ('::test-user'))
							.then (function (user) {
								var hacked_login = like (my ('::api:login'));
								hacked_login .user .uuid = user .uuid;
								return 	inquire (self .affiliated ('::api:logout'))
											.then (function () {
												return localforage .clear ()
											})
											.then (function () {
												return inquire (self .affiliated ('::api:login', hacked_login))
											})
							})
							.then (function () {
...
(function (self, args, my) {
		...
		mergeAll ([
			...
			.thru (map, function () {
				return inquire (self .affiliated ('::user'));
			})
			.thru (filter, function (user) {
				if (user .status === 'UNVERIFIED') {
					window .location .hash = '#user/verify';
					return false;
				}
				else
					return true;
			...
			.thru (tap, function () {
				return	inquire (self .affiliated ('::teams'))
							.then (function (teams) {
								if (teams [0])
									window .location .hash = home_path;
								else
						...
						openFB .login (
							function (authentication) {
								if (authentication && authentication .status === 'connected' && authentication .authResponse .accessToken) {
									...
									inquire (self .affiliated ('::login'), {
										access_token: login_info .accessToken,
										provider: 'Facebook'
									})
										.then (function (login) {

											self .mention ('::api:login', login);
										})
								}
								else {
...
(function (self, args, my) {
		...
		self
			...
			.establish ('home-shirt-color', dependent (function () {
				return my ('match-item') .home_team_color;
			}, self .impressions ('match-item')))
			.establish ('home-age-range', dependent (function () {
				var lower = my ('match-item') .home_team .age_group_lower;
				var upper = my ('match-item') .home_team .age_group_upper;
			...
			.establish ('away-age-range', dependent (function () {
				var lower = my ('team-item') .age_group_lower;
				var upper = my ('team-item') .age_group_upper;
			...
			.establish ('match-times', dependent (function () {
				...
				var duration = my ('match-item') .duration;
			...
			.establish ('match-pitch', dependent (function () {
				...
				return num_of_players_to_text (my ('match-item') .match_type_value) + ' - ' + pitch_type_to_chi (my ('match-item') .pitch_type);
			}, self .impressions ('match-item')))
		...
		self
			...
			.impressions ('apply-button') .thru (tap, function (ref) {
				ref .addEventListener ('click', function (e) {
					...
					var apply = my ('match-apply') (my ('team-item') .id, parse (args [0]));
		...
		self
			.impressions (args .cycle__from)
				.thru (tap, function (cycle) {
					if (self .affiliated ('match-to-find-item')) {
						var match = my ('match-to-find-item') (parse (args [0]))
					...
					inquire_last (self .affiliated ('::teams')) .then (function (teams) {
						...
						inquire (self .affiliated (match_open), {
					        start_at: 	fecha .format
					        				(fecha .parse (parse (args [0]) + ' ' + parse (args [1]), 'YYYY年M月D日 h:mm A'),
					        				'YYYY-MM-DDTHH:mm:ss+08:00'),
					        duration: parse (args [2]),
					        ...
					        home_team_color: parse (args [3]),
					        fee_per_team: parse (args [5]) || 0,
					        pitch_name: parse (args [4]),
					        match_type: + (parse (args [6]) .split ('v') [0]),
					        country: 'HKG',
					        city: 'Hong Kong',
					        region: 'Hong Kong'
						})
...
(function (self, args, my) {
		...
		self
			...
			.establish ('home-age-range', dependent (function () {
				var lower = my ('team-info') .age_group_lower;
				var upper = my ('team-info') .age_group_upper;
			...
			.establish ('match-time', dependent (function () {
				...
				var duration = my ('match-info') .duration;
			...
			.establish ('match-pitch', dependent (function () {
				...
				return num_of_players_to_text (my ('match-info') .match_type_value) + ' - ' + pitch_type_to_chi (my ('match-info') .pitch_type);
			}, self .impressions ('match-info')))
			.establish ('match-fee', dependent (function () {
...
riot.tag2('page-test-list', '<nav> <nav-bar> <nav-title> <component-page-title>建立球隊</component-page-title> </nav-title> </nav-bar> </nav> <component-main-content> <test-list> 接受球賽通知 <br>當球賽即將開始時，你願意接受球賽通知？ <component-tree-control items__from=":test" items__to=":test"></component-tree-control> </test-list> </component-main-content>', '', '', function(opts) {
(function (self, args, my) {
					...
					if ((phone_num + '') .length === 8) {
						return 	stream ({
									...
									.thru (map, function (phone_info) {
										return inquire (self .affiliated ('::contact'), phone_info)
									})
